...
export async function POST(request: NextRequest) {
  try {
    ...
    // Accept both mediaId and videoId for compatibility
    const mediaId = body.mediaId || body.videoId || 'unknown';
    const mediaTitle = body.mediaTitle || body.videoTitle || 'Instagram Post';
    ...
    // Record the download
    await recordDownload(user.id, mediaId, mediaTitle, comments.length, `instagram-${format}`);
    ...
    if (format === 'csv') {
      ...
      return new NextResponse(csv, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="instagram-comments-${mediaId}.csv"`,
        },
      });
    } else if (format === 'xlsx') {
      ...
      return new NextResponse(excel, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="instagram-comments-${mediaId}.xlsx"`,
        },
      });
    }
